...
for (const type of types) {
  const url = `https://raw.githubusercontent.com/SAP/abap-file-formats/main/file-formats/${type}/${type}-v1.json`;
  const raw = child_process.execSync(`curl -H "Accept: application/vnd.github+json" ${url}`);
  schemas += `schemas["${type}"] = ` + JSON.stringify(JSON.parse(raw.toString())) + `;\n`;
}
